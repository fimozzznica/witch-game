Shader "SyntyStudios/VegitationShader_URP"
{
    Properties
    {
    ...
    SubShader
    {
       // Tags{ "RenderType" = "TransparentCutout"  "Queue" = "Geometry+0" "IsEmissive" = "true" "RenderPipeline" = "UniversalPipeline" }
        Tags { 
        ...
        inline float DitherNoiseTex(float4 screenPos, TEXTURE2D_PARAM(noiseTexture, sampler_noiseTexture), float4 texelSize)
        {
            float2 uv = screenPos.xy * _ScreenParams.xy * texelSize.xy;
            float dither = SAMPLE_TEXTURE2D_LOD(noiseTexture, sampler_noiseTexture, uv, 0).g;
            return dither;
        }
        ENDHLSL
        ...
        Pass
        {
            ...
            TEXTURE2D(_LeafNormalMap);
            SAMPLER(sampler_LeafNormalMap);
            TEXTURE2D(_LeafAmbientOcclusion);
            SAMPLER(sampler_LeafAmbientOcclusion);
            TEXTURE2D(_TunkTex);
            SAMPLER(sampler_TunkTex);
            TEXTURE2D(_TrunkNormalMap);
            SAMPLER(sampler_TrunkNormalMap);
            TEXTURE2D(_TrunkAmbientOcclusion);
            SAMPLER(sampler_TrunkAmbientOcclusion);
            TEXTURE2D(_EmissiveMask);
            SAMPLER(sampler_EmissiveMask);
            TEXTURE2D(_Emissive2Mask);
            SAMPLER(sampler_Emissive2Mask);
            TEXTURE2D(_TrunkEmissiveMask);
            SAMPLER(sampler_TrunkEmissiveMask);
            TEXTURE2D(_EmissivePulseMap);
            SAMPLER(sampler_EmissivePulseMap);
            TEXTURE2D(_GustNoiseMap);
            SAMPLER(sampler_GustNoiseMap);
            TEXTURE2D(_Leaves_NoiseTexture2);
            SAMPLER(sampler_Leaves_NoiseTexture2);
            ...
            Varyings vert( Attributes v )
            {
                ...
                float3 ase_vertex3Pos = v.positionOS.xyz;
                float mulTime28 = _Time.y * (0.0 + (_JitterFreq - 0.0) * (0.06 - 0.0) / (1.0 - 0.0));
                float2 temp_cast_0 = (( ase_vertex3Pos.x + mulTime28 )).xx;
                float lerpResult50 = lerp( SAMPLE_TEXTURE2D_LOD(_Leaves_NoiseTexture2, sampler_Leaves_NoiseTexture2, temp_cast_0, 0).r , 0.0 , ( 1.0 - v.color.g ));
                ...
                float lerpResult130 = lerp(
                    (simplePerlin3D117 * sin((_LeafBigNoiseScale * (grayscale304 + _Time.y)))), 
                    0.0, 
                    (1.0 - v.color.g)
                );
                ...
                float temp_output_177_0 = ( (0.0 + (_GustAmount - 0.0) * (4.0 - 0.0) / (1.0 - 0.0)) * ( ( ( simplePerlin2D368 * sin( ( ( grayscale346 * _GustScale ) + ( _Time.y * -(0.0 + (_GustSmallFreq - 0.0) * (10.0 - 0.0) / (1.0 - 0.0)) ) ) ) ) + 0.5 ) / 2.0 ) * ( ( SAMPLE_TEXTURE2D_LOD(_GustNoiseMap, sampler_GustNoiseMap, temp_cast_4, 0).r + 0.5 ) / 2.0 ) );
                ...
                v.positionOS.xyz += ( Jitter68 + float4( Wind201 , 0.0 ) ).rgb;
                
                o.positionWS = ase_worldPos;
            ...
            half4 frag(Varyings i) : SV_Target
            {
                ...
                float2 uv_LeafNormalMap = i.uv * _LeafNormalMap_ST.xy + _LeafNormalMap_ST.zw;
                float2 uv_TrunkNormalMap = i.uv * _TrunkNormalMap_ST.xy + _TrunkNormalMap_ST.zw;
                float3 normal = (i.color.b > 0.5 ? 
                    UnpackNormalScale(SAMPLE_TEXTURE2D(_LeafNormalMap, sampler_LeafNormalMap, uv_LeafNormalMap), _LeafNormalAmount) : 
                    UnpackNormalScale(SAMPLE_TEXTURE2D(_TrunkNormalMap, sampler_TrunkNormalMap, uv_TrunkNormalMap), _TrunkNormalAmount));
                
                float2 uv_LeafTex = i.uv * _LeafTex_ST.xy + _LeafTex_ST.zw;
                float2 uv_TunkTex = i.uv * _TunkTex_ST.xy + _TunkTex_ST.zw;
                float4 temp_output_551_0 = ( i.color.b > 0.5 ? SAMPLE_TEXTURE2D(_LeafTex, sampler_LeafTex, uv_LeafTex) : SAMPLE_TEXTURE2D(_TunkTex, sampler_TunkTex, uv_TunkTex) );
                float3 ase_worldPos = i.positionWS;
                ...
                float temp_output_177_0 = 
                    (lerp(0.0, 4.0, _GustAmount)) * 
                    (
                        (
                            (
                                simplePerlin2D368 * 
                                sin(
                                    (grayscale346 * _GustScale) + 
                                    (_Time.y * -lerp(0.0, 10.0, _GustSmallFreq))
                                ) + 
                                0.5
                            ) / 2.0
                        ) * 
                        (
                            (SAMPLE_TEXTURE2D(_GustNoiseMap, sampler_GustNoiseMap, temp_cast_3).r + 0.5) / 2.0
                        )
                    );             
                float gustValue373 = temp_output_177_0;
                float4 lerpResult378 = lerp((_GustHighlight * gustValue373), float4(0,0,0,0), (1.0 - i.color.g));
                float4 temp_output_379_0 = ((_LeafFlatColourSwitch) ? lerpResult497 : max(blendOpSrc73, blendOpDest73)) + lerpResult378;    
                float3 ase_worldNormal = normalize(i.normalWS);
                float frostingFactor = saturate(pow(max(0, ase_worldNormal.y), _FrostingHeight) * _FrostingFalloff);
                float4 lerpResult461 = lerp(temp_output_379_0, _FrostingColour, frostingFactor);   
                ...
                float4 Albedo78 = ( i.color.b > 0.5 ? (( _FrostingSwitch )?( lerpResult461 ):( temp_output_379_0 )) : (( _TrunkFlatColourSwitch )?( lerpResult570 ):( max( blendOpSrc575, blendOpDest575 ) )) );
                float4 albedo = ( i.color.b > 0.5 ? (( _FrostingSwitch )?( lerpResult461 ):( temp_output_379_0 )) : (( _TrunkFlatColourSwitch )?( lerpResult570 ):( max( blendOpSrc575, blendOpDest575 ) )) );

                float2 uv_EmissiveMask = i.uv * _EmissiveMask_ST.xy + _EmissiveMask_ST.zw;
                float4 lerpResult417 = lerp( _EmissiveColour , float4( 0,0,0,0 ) , ( 1.0 - SAMPLE_TEXTURE2D(_EmissiveMask, sampler_EmissiveMask, uv_EmissiveMask).r ));
                float2 uv_Emissive2Mask = i.uv * _Emissive2Mask_ST.xy + _Emissive2Mask_ST.zw;
                float4 lerpResult457 = lerp( _Emissive2Colour , float4( 0,0,0,0 ) , ( 1.0 - SAMPLE_TEXTURE2D(_Emissive2Mask, sampler_Emissive2Mask, uv_Emissive2Mask).r ));
                float2 uv_TrunkEmissiveMask = i.uv * _TrunkEmissiveMask_ST.xy + _TrunkEmissiveMask_ST.zw;
                float4 lerpResult569 = lerp( _TrunkEmissiveColour , float4( 0,0,0,0 ) , ( 1.0 - SAMPLE_TEXTURE2D(_TrunkEmissiveMask, sampler_TrunkEmissiveMask, uv_TrunkEmissiveMask).r ));
                float2 temp_cast_5 = (0.03).xx;
                float mulTime436 = _Time.y * 0.04;
                float2 temp_cast_6 = (mulTime436).xx;
                float2 uv_TexCoord435 = i.uv * temp_cast_5 + temp_cast_6;
                ...
                float clampResult444 = clamp( max( SAMPLE_TEXTURE2D(_EmissivePulseMap, sampler_EmissivePulseMap, uv_TexCoord435).r , grayscale432 ) , 0.0 , 1.0 );
                float4 Emissive420 = ( ( i.color.b > 0.5 ? ( lerpResult417 + lerpResult457 ) : lerpResult569 ) * _EmissiveAmount * (0.5 + (clampResult444 - 0.0) * (1.0 - 0.5) / (1.0 - 0.0)) );

                float temp_output_189_0 = ( 1.0 - i.color.b );
                float lerpResult188 = lerp( _LeafMetallic , 0.0 , temp_output_189_0);
                                float lerpResult195 = lerp( _TrunkMetallic , 0.0 , i.color.b);
                float metallic = ( lerpResult188 + lerpResult195 );
                
                float lerpResult191 = lerp( _LeafSmoothness , 0.0 , temp_output_189_0);
                float lerpResult190 = lerp( _TrunkSmoothness , 0.0 , i.color.b);
                float smoothness = ( lerpResult191 + lerpResult190 );
                
                float2 uv_LeafAmbientOcclusion = i.uv * _LeafAmbientOcclusion_ST.xy + _LeafAmbientOcclusion_ST.zw;
                float2 uv_TrunkAmbientOcclusion = i.uv * _TrunkAmbientOcclusion_ST.xy + _TrunkAmbientOcclusion_ST.zw;
                float occlusion = ( i.color.b > 0.5 ? SAMPLE_TEXTURE2D(_LeafAmbientOcclusion, sampler_LeafAmbientOcclusion, uv_LeafAmbientOcclusion).r : SAMPLE_TEXTURE2D(_TrunkAmbientOcclusion, sampler_TrunkAmbientOcclusion, uv_TrunkAmbientOcclusion).r );
                
                float Alpha77 = temp_output_551_0.a;
                float4 ase_screenPosNorm = i.screenPosition / i.screenPosition.w;
                ase_screenPosNorm.z = (UNITY_NEAR_CLIP_VALUE >= 0) ? ase_screenPosNorm.z : ase_screenPosNorm.z * 0.5 + 0.5;
                TEXTURE2D_ARGS(_LOD_DitheringMap, sampler_LOD_DitheringMap);            
                #ifdef LOD_FADE_CROSSFADE
                    float staticSwitch533 = 1.0;
                #else
                    // Исправленный lerp с правильными скобками
                    float staticSwitch533 = (unity_LODFade.x < 0.0) ? 
                ...
                #else
                    float staticSwitch532 = (unity_LODFade.x > 0.0) ? 
                        (sqrt(unity_LODFade.x) / 0.7) : 
                        1.0;
                #endif
                SurfaceData surface;
                ...
                #else
                    surface.normalTS = float3(0, 0, 1); // Нормаль по умолчанию если нет карты нормалей
                #endif
                surface.emission = Emissive420.rgb;
                surface.metallic = metallic;
                surface.smoothness = smoothness;
                surface.occlusion = occlusion;
                surface.alpha = Alpha77;
                surface.specular = 0.0; // Исправлено: используем surface вместо surfaceData
                surface.clearCoatMask = 0.0; // Исправлено: используем surface вместо surfaceData
                surface.clearCoatSmoothness = 0.0; // Исправлено: используем surface вместо surfaceData
                ...
                inputData.bakedGI = 0;
                inputData.normalizedScreenSpaceUV = GetNormalizedScreenSpaceUV(i.positionCS); // Исправлено: используем i.positionCS
                ...
                #ifdef LOD_FADE_CROSSFADE
                    float dither = InterleavedGradientNoise(i.positionWS.xy, 0);
                    clip(color.a - dither);
                #endif
        ...
        Pass
        {
            ...
            half4 ShadowPassFragment(Varyings i) : SV_TARGET
            {
                ...
                // Используем dither535 вместо dither
                float dither535 = DitherNoiseTex(ase_screenPosNorm, TEXTURE2D_ARGS(_LOD_DitheringMap, sampler_LOD_DitheringMap), _LOD_DitheringMap_TexelSize);
